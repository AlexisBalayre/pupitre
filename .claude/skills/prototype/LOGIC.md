...
Write it for a non-developer. Every label is in **domain language** (the project's own terms: the `SCHEMA` nouns in `src/core/db.client.ts`, `docs/09-decisions.md`), not code: buttons and state read like the business, not the reducer. Explain in plain words what's happening.
