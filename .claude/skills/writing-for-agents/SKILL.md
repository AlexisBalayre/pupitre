...
- **Skill names are kebab-case and match their directory**; the `name` frontmatter equals the folder name. Gerund-style names (`diagnosing-bugs`, `writing-for-agents`) and imperative ones (`improve-codebase-architecture`) are both fine.
- **Hardcode repo specifics; source machine specifics from config.** Skills bake in doc paths and take the project's commands from `.claude/project.env` (`LINT_CMD`, `TYPECHECK_CMD`, `TEST_CMD`, ...) rather than naming a toolchain; per-machine values (vault paths, personal IDs) come from `.env` keys (see `.env.example`).
- **Point at the repo's docs, never a parallel store.** The design lives in `docs/00`–`08`, the resolved decisions in `docs/09-decisions.md` (which wins over `00`–`08`), the coding rules in `docs/conventions/`, and the domain nouns in the `SCHEMA` string of `src/core/db.client.ts`; documents reference those (the way `domain-modeling` does), and never create a `CONTEXT.md` or a glossary.
