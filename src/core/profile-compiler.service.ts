 ...
 */
/**
 * Past this many dirty paths the re-check refuses rather than read them all: it
 * forks per path, twice per call, under a hook timeout that fails open.
 */
const RECHECK_MAX_DIRTY_PATHS = 200;
/** A file larger than this is fingerprinted by its size, not read whole. */
const RECHECK_HASH_MAX_BYTES = 64 * 1024 * 1024;
...
# PostToolUse and PostToolUseFailure(Bash), PostToolUse(BashOutput|TaskOutput): any
...
out_of_scope() {
  ...
  COUNT=$(( $(cat "$WORK/status" "$WORK/protected" | tr -dc '\\000' | wc -c) ))
  [ "$COUNT" -le ${RECHECK_MAX_DIRTY_PATHS} ] || return 2
  { tr '\\n\\000' '\\001\\n' < "$WORK/status"
    tr '\\n\\000' '\\001\\n' < "$WORK/protected" | grep '^!! '
...
export function compileProfile(input: CompileInput): CompiledProfile {
  ...
  // output is the next point a session looks at it, so the check runs again there.
  const securityPostBackground: HookMatcherEntry[] = [
    { matcher: 'BashOutput|TaskOutput', hooks: [bashRecheck] },
  ];
