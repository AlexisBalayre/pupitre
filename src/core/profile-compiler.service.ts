...
function buildBashRecheckScript(input: CompileInput): string {
  const hooksDir = join(input.outDir, 'hooks');
  return `#!/bin/sh
# PreToolUse, PostToolUse and PostToolUseFailure(Bash) write re-check — generated by
# the Pupitre profile compiler (decision 63). Before the call: snapshot every dirty
# path outside this task's scope. After it: any such path that is new or changed
# since the snapshot is recorded as a scope_violation and refused loudly. The merge
# gate's diff-vs-scope audit stays the authoritative backstop (decision 6).
INPUT=$(cat)
...
# git names every path raw with -z; tr turns each newline inside a name into \\001
# before NUL becomes the line separator, so a record is always one line and a name
# is only ever quoted data, never split, globbed or evaluated. Precedence is the
# scope hook's: .claude/, then scope-out, then scope-in.
out_of_scope() {
  git -C "$WORKTREE" -c core.fsmonitor= status --porcelain=v1 -z --untracked-files=all \\
    --no-renames > "$WORK/status" || return 1
  tr '\\n\\000' '\\001\\n' < "$WORK/status" | while IFS= read -r ENTRY; do
    CODE=\${ENTRY%"\${ENTRY#??}"}
    FILE=\${ENTRY#???}
...
export function compileProfile(input: CompileInput): CompiledProfile {
  ...
  const settings = {
    hooks: {
      ...
      PostToolUse: [...eventEntry, ...securityPostBash, ...(layerHooks.PostToolUse ?? [])],
